...
//           <input type="search" placeholder="Search movies..." className="search-input" />
//         </div>
//         <nav className="nav-bar">
//           <a href="#" onClick={() => onNavigate("home")} className="nav-link">
//             Movies
...
export function Header({ onNavigate }: HeaderProps) {
  ...
  // Thêm state cho ô tìm kiếm trong dropdown
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      if (genreDropdownRef.current && !genreDropdownRef.current.contains(event.target as Node)) {
        ...
        setGenreSearchQuery("") // Reset search query when closing
      }
      if (countryDropdownRef.current && !countryDropdownRef.current.contains(event.target as Node)) {
        setShowCountryDropdown(false)
        setCountrySearchQuery("") // Reset search query when closing
      }
    }
  ...
  const handleGenreClick = (genreId: number, genreName: string) => {
    ...
    console.log(`Toggled genre: ${genreName} (ID: ${genreId})`)
  }
  ...
  const handleCountryClick = (countryCode: string, countryName: string) => {
    ...
    console.log(`Toggled country: ${countryName} (Code: ${countryCode})`)
  }
          ...
          <div className="nav-link-dropdown" ref={genreDropdownRef}>
            <button
              ...
              onClick={() => {
                ...
                setShowCountryDropdown(false)
                setGenreSearchQuery("") // Reset search query when opening
              }}
