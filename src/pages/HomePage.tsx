...
//         </div>
//       ) : (
//         <HeroSection movie={heroMovie} movieGenres={movieGenres} />
//       )}
...
export function HomePage({ onNavigateToDetail }: HomePageProps) {
  const [heroMovies, setHeroMovies] = useState<MediaItem[]>([]) // Thay đổi từ heroMovie sang heroMovies (array)
  const [currentHeroMovieIndex, setCurrentHeroMovieIndex] = useState(0) // State mới để quản lý chỉ mục phim hero
  const [movieGenres, setMovieGenres] = useState<{ [key: number]: string }>({})
  const [trendingMovies, setTrendingMovies] = useState<MediaItem[]>([])
  const [popularMovies, setPopularMovies] = useState<MediaItem[]>([])
  const [loading, setLoading] = useState(true)
